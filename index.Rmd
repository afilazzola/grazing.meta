...
source("meta.eval.r") ## Multiple aggregate
  ...
  m2 <- rma(yi=yi, vi=vi, mods=~ -1 + previously.grazed,  data = meta.ready)
  summary(m2) 
...
source("meta.eval.r") ## Multiple aggregate
  ...
  ## Select only the producers
  meta2 <- meta2 %>% filter(Functional.Group == "Producer") ##  plants, fungi and mychoriza 
  ...
  ##write.csv(meta.ready, "data//effectSize//abd.plant.csv", row.names=FALSE)
```
...
preds <- predict(m3, newmods=c(-5:40), transf=exp)
...
plot(NA, NA, xlim=c(-5,30), ylim=c(0.2,10),
     xlab="Temperature (°C)", ylab="Risk Ratio",
     las=1, bty="l", log="y")

### add points
symbols(meta.ready$bio1, exp(meta.ready$yi), circles=size, inches=FALSE, add=TRUE, bg="black")
                   
### add predicted values (and corresponding CI bounds)
lines(-5:40, preds$pred)
lines(-5:40, preds$ci.lb, lty="dashed")
lines(-5:40, preds$ci.ub, lty="dashed") 
...
## get last year grazed as covariate
meta.ready <- subset(meta3, yr.grazed>0)
...
## Temperature
### calculate predicted risk ratios for full temperature gradient
preds <- predict(m3, newmods=c(0:100), transf=exp)
...
### set up plot (risk ratios on y-axis, absolute temperature on x-axis)
plot(NA, NA, xlim=c(0,100), ylim=c(0.2,10),
