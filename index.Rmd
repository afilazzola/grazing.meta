...
## time-since-grazed
m1.tsg <- rma(yi=yi, vi=vi,  mods=~ yr.grazed, data = subset(plt, measure == "abundance" & vi!=0))
summary(m1.tsg) 
...
## time-since-grazed
m2.tsg <- rma(yi=yi, vi=vi, mods=~ yr.grazed , data = subset(plt, measure == "diversity" & vi!=0))
summary(m2.tsg)
...
```{r}
temp <- stack(list.files("F:\\worldclim2\\tmax\\", pattern=".tif", full.names=T))
temp <- mean(temp)
prec <- stack(list.files("F:\\worldclim2\\prec\\", pattern=".tif", full.names=T))
prec <- sum(prec)
...
## load rasters
prec <- raster("F:\\worldclim2\\bioclim\\bio1.tif")
...
### Temperature only
#mixed-effects meta-analysis for grazed vs ungrazed plots
m3 <- rma(yi=yi, vi=vi, mods=~ poly(bio1,2),  data = meta.ready)
summary(m3) 
...
geom_ribbon(data=data.frame(preds), aes(x=meta.ready$bio1, y=pred, ymin=ci.lb, ymax=ci.ub), alpha=0.2) + scale_color_manual(values=cbPalette)+xlim(0,40)
...
trophicReduced <- data.frame(FG=c("InvertebrateHerbivorous", "InvertebrateParasitic", "InvertebratePredator", "InvertebratePollinator", "VertebrateOmnivorous", "VertebratePredator", "VertebrateHerbivorous", "InvertebrateDetritivores", "InvertebrateOmnivorous"), trophic=c("Herbivore","Parasite","Predator","Pollinator","Omnivore","Predator","Herbivore","Detritivore","Omnivore"))

meta.ready <- merge(meta.ready, trophicReduced, by="FG")
## line
preds <- predict(m3, newmods=1:40)

cbPalette <- c("#77564C", "#4381C1", "#9AD4D6", "#4FB286", "#564787", "#08415C")


## animals
ggplot(meta.ready, aes(x=yr.grazed, y=yi))+ geom_hline(yintercept=0, lwd=1, lty=2)+ geom_jitter(aes(color=trophic, shape=higher.taxa), size=4, width=1) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), text = element_text(size=16)) + xlab("Time since control was last grazed (log-years)") + ylab("Effect of grazing relative to control (LRR)") + geom_line(data=data.frame(preds), aes(x=1:40, y=pred), lwd=1) +
geom_ribbon(data=data.frame(preds), aes(x=1:40, y=pred, ymin=ci.lb, ymax=ci.ub), alpha=0.2) + scale_color_manual(values=cbPalette)
