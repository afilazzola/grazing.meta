...
## abundance
m1 <- rma(yi=yi, vi=vi, data = subset(meta.ready, measure == "abundance" & vi!=0) , test="t" )
summary(m1) 
...
## time-since-grazed
m1.tsg <- rma(yi=yi, vi=vi,  mods=~ yr.grazed, data = subset(meta.ready, measure == "abundance" & vi!=0), test="t")
summary(m1.tsg) 

## richness
m2 <- rma(yi=yi, vi=vi, data = subset(meta.ready, measure == "diversity"& vi!=0) , test="t")
summary(m2) 
...
## time-since-grazed
m2.tsg <- rma(yi=yi, vi=vi, mods=~ poly(yr.grazed,2) , data = subset(meta.ready, measure == "diversity" & vi!=0), test="t")
summary(m2.tsg)
...
### Temperature only
#mixed-effects meta-analysis for grazed vs ungrazed plots
m3 <- rma(yi=yi, vi=vi, mods=~ poly(temp,2) * poly(precip,1),  data = meta.ready , test="t" )
summary(m3) 
...
## Model
m1 <- rma(yi=yi, vi=vi, mods=~  poly(plteff,1),  data = pltabd, test="t")
summary(m1)
...
## predict line
preds <- predict(m1, plteff=pltabd$plteff)

##plot
ggplot(data=pltabd, aes(x=plteff, y=yi))+ geom_hline(yintercept=0, lwd=1, lty=2)+ geom_point(aes(color=trophic, shape=higher.taxa), size=4) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), text = element_text(size=16)) + xlab("Effect of livestock on plants (LRR)") + ylab("Effect of livestock on animals (LRR)") + geom_line(data=data.frame(preds), aes(x=pltabd$plteff, y=pred), lwd=1) +
geom_ribbon(data=data.frame(preds), aes(x=pltabd$plteff, y=pred, ymin=ci.lb, ymax=ci.ub), alpha=0.2) + scale_color_manual(values=cbPalette)+ 
  annotate("text", x=-.2, y=.6, label="plant div - animal div", size=6)
